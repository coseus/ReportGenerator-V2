...
def render_executive_summary_tab(report_data):
    ...
    if not findings:
        ...
        report_data["executive_summary"] = st.text_area(
            ...
            value=report_data.get("executive_summary", ""),
            height=200,
    ...
    st.caption("Acest text apare in raport ca Prezentare executiva (sectiunea 2.0). "
               "Daca il lasi gol, raportul genereaza automat un rezumat din constatari.")
    report_data["executive_summary"] = st.text_area(
        "Custom Executive Summary (optional)",
        value=report_data.get("executive_summary") or default_summary,
        height=200,
        key="executive_summary",
    )
